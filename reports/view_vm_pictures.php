...
if($stmt = $conn->prepare($sql_query)){
    ...
    if($stmt->execute()){
        ...
        while($row = $result->fetch_assoc()) { $vm_reports[] = $row; }
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2>View VM Picture Submissions</h2>
...
<div class="card mb-4 no-print">
    <div class="card-body">
        <form action="reports/view_vm_pictures.php" method="get" class="row g-3 align-items-end">
            <?php if($is_manager): ?>
            <div class="col-md-4">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-striped table-hover table-sm">
                ...
                <tbody>
                    <?php if (empty($vm_reports)): ?>
                        <tr><td colspan="8" class="text-center text-muted">No VM reports found for this period.</td></tr>
                    <?php else: ?>
                        <?php foreach($vm_reports as $item): ?>
                        <tr>
                            ...
                            <td><?php echo $item['photo_url_1'] ? '<a href="'.$item['photo_url_1'].'" target="_blank">View</a>' : 'N/A'; ?></td>
                            <td><?php echo $item['photo_url_2'] ? '<a href="'.$item['photo_url_2'].'" target="_blank">View</a>' : 'N/A'; ?></td>
                            <td><?php echo $item['photo_url_3'] ? '<a href="'.$item['photo_url_3'].'" target="_blank">View</a>' : 'N/A'; ?></td>
                            <td><?php echo $item['photo_url_4'] ? '<a href="'.$item['photo_url_4'].'" target="_blank">View</a>' : 'N/A'; ?></td>
