...
$sql_query = "SELECT 
                ...
                u.full_name, 
                s.store_name 
              FROM report_competition_data r
...
$sql_query .= " ORDER BY r.report_date DESC, r.competitor_name";

if($stmt = $conn->prepare($sql_query)){
    $stmt->bind_param($types, ...$params);
    if($stmt->execute()){
        $result = $stmt->get_result();
        while($row = $result->fetch_assoc()) { $comp_data[] = $row; }
...
<div class="card mb-4 no-print">
    <div class="card-body">
        <form action="reports/view_competition.php" method="get" class="row g-3 align-items-end">
            <?php if($is_manager): ?>
            <div class="col-md-4">
                <label for="store_id" class="form-label">Store (Reference)</label>
...
<div class="card printable-area">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4>Competition Data Submissions</h4>
            ...
            <table class="table table-striped table-hover table-sm">
                ...
                <tbody>
                    <?php if (empty($comp_data)): ?>
                        <tr><td colspan="7" class="text-center text-muted">No competition data found for this period.</td></tr>
                    <?php else: ?>
                        <?php foreach($comp_data as $item): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($item['report_date']); ?></td>
                            <td><strong><?php echo htmlspecialchars($item['competitor_name']); ?></strong></td>
                            <td><?php echo htmlspecialchars($item['activity_type']); ?></td>
                            <td>
                                <?php echo htmlspecialchars($item['offer_details']); ?>
                                <small class="d-block text-muted"><i><?php echo htmlspecialchars($item['remarks']); ?></i></small>
                            </td>
                            <td><?php echo htmlspecialchars($item['full_name']); ?></td>
                            <td><?php echo htmlspecialchars($item['store_name']); ?></td>
                            <td><?php echo $item['photo_url'] ? '<a href="'.$item['photo_url'].'" target="_blank">View</a>' : 'N/A'; ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
